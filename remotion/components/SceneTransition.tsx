...
export const SceneTransition: React.FC<SceneTransitionProps> = ({
  ...
  const transitionDuration = Math.max(1, Math.min(safeDuration - 1, Math.round(fps * 0.5)));
  ...
  const exitStart = Math.max(0, durationFrames - transitionDuration);
  const exitEnd = Math.max(exitStart + 1, durationFrames);
  const exitProgress = interpolate(frame, [exitStart, exitEnd], [1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  ...
  switch (type) {
    ...
    case "wipeLeft":
      ...
      const clipExit = interpolate(exitProgress, [0, 1], [100, 0]);
    ...
    case "slideLeft":
      ...
      const slideExitX = interpolate(exitProgress, [0, 1], [-100, 0]);
    ...
    case "slideUp":
      ...
      const slideExitY = interpolate(exitProgress, [0, 1], [-100, 0]);
    ...
    case "zoom":
      ...
      const zoomExit = interpolate(exitProgress, [0, 1], [0.5, 1]);
